        ...
        return new Promise((resolve, reject) => {
            ...
            this.pool.query("CREATE TABLE IF NOT EXISTS current (id VARCHAR(20) NOT NULL, name VARCHAR(200) NOT NULL, description VARCHAR(500) NOT NULL, state BOOLEAN NOT NULL)", (error, results, fields) => {
                if (this.checkError(error)) { reject() }
                    ...
                    this.pool.query("CREATE TABLE IF NOT EXISTS `query` ( `id` VARCHAR(20) NOT NULL, `playersOn` INT NOT NULL, `playersMax` INT NOT NULL, `ping` INT NOT NULL, `players` TEXT);", (error, results, fields) => {
                        if (this.checkError(error)) { reject() }
                        else { 
            ...
            this.pool.query("SELECT id FROM current", (error, results, fields) => {
                if (this.checkError(error)) { reject() }
                    ...
                    if (!results) {
                        ...
                        let remove = results.filter((res) => { if (!config.servers.includes(res.id)) { return true } } );
                        remove.forEach((res) => {
